...
class Task implements TaskRepositoryInterface {
    ...
    public function getAllByUserId($user_id) {
        // Fetches tasks for the specific logged-in user, sorted by ID
        return $this->db->select()
                        ->from('tasks')
                        ->where('user_id', $user_id)
                        ->orderBy('id', 'ASC') // Ensures numerical order 1, 2, 3...
                        ->get();
    }

    public function getAll() {
        // Fetches all tasks in the system, sorted by ID
        return $this->db->select()
                        ->from('tasks')
                        ->orderBy('id', 'ASC') // Ensures numerical order 1, 2, 3...
                        ->get();
    ...
    public function createWithManualId($id, $title, $user_id, $description) {
        // Allows you to manually set the ID (e.g., ID 10 for Midterm)
        return $this->db->table('tasks')
                        ->insert([
                            'id' => $id, 
                            'title' => $title,
